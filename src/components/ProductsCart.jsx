...
const ProductsCart = ({ cartList, isCartOpen, setIsCartOpen, delProduct, qtyProduct, checkout }) => {
    ...
    const getTotal = (cartList) => {
        ...
        cartList.map(cartProduct => {
            ...
            total += Number(cartProduct.productsInCart.quantity) * Number(cartProduct.price)
        });
        return total
    }
        ...
        <Container>
            <div className={`cart-modal ${isCartOpen ? 'open' : ''}`}>
                ...
                <Row>
                    <Col>
                        <br />
                        <h5>Totals :{getTotal(cartList)}</h5>

                        <button onClick={() => checkout(getTotal(cartList))} className="btn btn-info btn-checkout" >Checkout</button>
                        <br />
                    </Col>
                </Row>
            </div>
